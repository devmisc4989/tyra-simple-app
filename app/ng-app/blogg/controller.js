(function() {
  ...
  angular.module('tyraApp.blogg').config([
    'ParseProvider', '$routeProvider', function(ParseProvider, $routeProvider) {
  ...
  angular.module('tyraApp.blogg').controller('bloggController', function($scope, $rootScope, Blogg) {
    $rootScope.title = 'Tyra App | Blogg';
    $rootScope.bodyClass = 'skin-blue';
    $scope.load = function() {
      return Blogg.query({
        include: 'images,blogg_author',
        order: '-createdAt'
      }).then(function(blogg_docs) {
        return $scope.blogg_docs = blogg_docs;
      });
    };
    return $scope.load();
  });
